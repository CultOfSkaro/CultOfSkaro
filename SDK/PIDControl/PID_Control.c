...
void setVelocity(int32 velocity){
	if(velocity > 5500.0)
    	velocity = 5500.0;
    else if(velocity < -5500.0)
	    velocity = -5500.0;
	pid.desiredVelocityPID = velocity;
}

void updateDistanceSetVelocity(){
	pid.distanceError = pid.desiredDistancePID - getTicks();
	int distanceError = pid.distanceError;
	int velocity = pid.desiredVelocityPID;
