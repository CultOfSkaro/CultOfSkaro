...
#define MEMORY_ADDRESS 0x007A2558 //0x81d00000  //XPAR_MPMC_0_MPMC_BASEADDR
...
void CameraISR()
{
	...
	if(frameCount == 100)
	{
		...
		USB_blockWrite((u32*)MEMORY_ADDRESS, 640*480);
		USB_forceCommit();

		print("wrote image to usb\r\n");
		*/
		...
		{
			result = XIo_In32(MEMORY_ADDRESS + i);
			//xil_printf("%x\r\n", result );
			xil_sprintf(line, "%08x\r\n", result);
			char *c;
			for (c = line; *c; c += 2) {
				USB_singleWrite(*((u16*)c));
			}
			USB_forceCommit();
			/*
