...
#define MEMORY_ADDRESS 0x007A2558 //XPAR_MPMC_0_MPMC_BASEADDR
...
void CameraISR()
{
	...
	Xuint32 status = XIntc_mGetIntrStatus(XPAR_INTC_SINGLE_BASEADDR);
	...
	{
		result = XIo_In32((Xuint32*)(MEMORY_ADDRESS2) + i);
	...
	if(frameCount == 99)
	  {
	    SetFrameAddress(VISION_FRAME_RGB565, MEMORY_ADDRESS2);
	  }
	StartFrameCapture();
	//Clear interrupt #1 flag
	XIntc_mAckIntr(XPAR_INTC_SINGLE_BASEADDR, "XPAR_PLB_VISION_0_INTERRUPT_MASK");
	//gotInt = 1;
...
int main()
{
	...
	XIntc_mEnableIntr( XPAR_XPS_INTC_0_BASEADDR, XPAR_PLB_VISION_0_INTERRUPT_MASK);
